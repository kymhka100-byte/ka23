...
# ---------------- 한글 폰트 ----------------
FONT_CANDIDATES = [
    "NanumGothic.ttf",             # 프로젝트 폴더에 폰트 파일 배치 시 지원
    "NotoSansKR-Regular.ttf",
    "C:/Windows/Fonts/malgun.ttf", # Windows 시스템 폰트
    "/system/fonts/NotoSansCJK-Regular.ttc", # 안드로이드 시스템 폰트
    ...
    try:
        with wave.open(path, "wb") as f:
            ...
            for i in range(frames):
                ...
                f.writeframesraw(value.to_bytes(2, "little", signed=True))
...
class GameButton(Button):
    ...
    def draw(self, *args):
        ...
        self.main_color.rgba = self.down_color if down else self.base
        self.main.pos = (x, y)
...
class RPSGame(BoxLayout):
    def __init__(self, **kwargs):
        ...
        for choice in CHOICES:
            ...
            b.bind(on_press=lambda instance, c=choice: self.choose(c))
    ...
    def later(self, function, seconds):
        def wrapper(dt):
            function()
        event = Clock.schedule_once(wrapper, seconds)
    ...
    def start_round(self, *args):
        ...
        self.later(lambda: self.set_message(COUNTDOWN[1]), 0.7)
        self.later(lambda: self.set_message(COUNTDOWN[2]), 1.4)
    ...
    def toggle_auto(self, *args):
        ...
        self.auto.text = "자동: ON" if self.auto_mode else "자동: OFF"
    ...
    def end_game(self):
        ...
        self.score.text = f"최종 점수  {self.win} : {self.lose}"
